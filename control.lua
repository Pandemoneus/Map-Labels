...
local function CreateMapLabelsForRegion( surface, region, resourceName, displayName )
  ...
  for i = 1, #fields do
    ...
    if not field.ignore then
      ...
      local mapLabel = surface.create_entity( { name = "map-label", force = "neutral", position = position } )--game.forces.player
      if mapLabel then
        mapLabel.backer_name = displayName
        mapLabel.destructible = false
        game.forces.player.chart( surface, { position, position } )
      end
    end
  end
end
...
local function DestroyMapLabels()
  for _, surface in pairs( game.surfaces ) do
    for chunk in surface.get_chunks() do
      ...
      for _, entity in ipairs( surface.find_entities_filtered( { name = "map-label", area = { { x, y }, { x + ChunkSize, y + ChunkSize } } } ) ) do
